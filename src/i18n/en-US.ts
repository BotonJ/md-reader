import type { MessageSchema } from "./zh-CN";

const enUS = {
  app: {
    file: "File",
    folder: "Folder",
    refresh: "Refresh",
    closeFolder: "Close folder",
    noFile: "No file opened",
    files: "Files",
    search: "Search",
    noFolder: "No folder opened",
    openFolderHint: 'Click "Folder"\nto browse Markdown files',
    emptyTitle: "MD Reader",
    emptyHint:
      'Click "File" or "Folder" to get started\nYou can also drag a .md file into this window',
    shortcutHint:
      "Ctrl+N New · Ctrl+O Open File · Ctrl+E Preview/Edit · Ctrl+F Find · Ctrl+S Save · Ctrl+Shift+F Search",
    toggleSidebar: "Toggle sidebar",
    toggleToc: "Toggle outline",
    toggleTheme: "Toggle theme",
    switchLanguage: "切换到中文",
    allFiles: "All files",
    usePath: "Use {path}",
    specifyEdgePath: "You can choose the Edge path when exporting",
    recentFiles: "Recent Files",
    clearRecent: "Clear List",
    openContainingFolder: "Open Containing Folder",
  },
  toolbar: {
    new: "New",
    find: "Find",
    export: "Export",
    settings: "Reading settings",
    settingsBtn: "Settings",
    sidebar: "Sidebar",
    outline: "Outline",
    print: "Print / System PDF",
  },
  find: {
    placeholder: "Find in current document",
    caseSensitive: "Case sensitive",
    previous: "Previous",
    next: "Next",
    close: "Close",
  },
  search: {
    placeholder: "Search in current folder",
    searching: "Searching…",
    noMatches: "No matches",
    typeToSearch: "Type to search",
    matches: "matches",
    files: "files",
    openFolderFirst: "Please open a folder first",
  },
  settings: {
    title: "Reading Settings",
    tabReading: "Reading",
    tabPdf: "PDF Export",
    fontSize: "Font Size",
    editorFontSize: "Editor Font Size",
    editorFontFamily: "Editor Font",
    lineHeight: "Line Height",
    maxWidth: "Page Width",
    fontFamily: "Body Font",
    tocPosition: "Outline Position",
    tocLeft: "Left",
    tocRight: "Right",
    system: "System",
    sans: "Sans",
    serif: "Serif",
    mono: "Mono",
    reset: "Reset",
    readerBg: "Reader Background",
    readerBgLight: "Light theme background",
    readerBgDark: "Dark theme background",
    readerBgReset: "Reset",
    readerBgResetAll: "Restore theme default background",
    presetDefaultLight: "Default White",
    presetPaper: "Paper",
    presetEyeCare: "Eye Care Green",
    presetLightYellow: "Light Yellow",
    presetDefaultDark: "Default Black",
    presetDarkSlate: "Dark Slate",
    presetDarkWarm: "Warm Black",
    updateCheck: "Software update",
    currentVersion: "Current version: {version}",
    checkUpdate: "Check for updates",
    checkingUpdate: "Checking…",
    updateAvailable: "New version {version} is available",
    upToDate: "You are up to date ({version})",
    updateCheckFailed: "Failed to check for updates",
    openReleasePage: "Open download page",
    versionUnavailable: "Unable to read current version",
    pandocTemplate: "Word export template",
    pandocTemplateHint:
      "Choose a .docx as reference document to control the font, heading and paragraph styles of exported Word files. Leave empty for pandoc defaults.",
    chooseTemplate: "Choose template",
    clearTemplate: "Clear",
    fileAssociation: "File association",
    fileAssociationHint:
      "Register MD Reader in the current Windows user's .md / .markdown / .mdx Open With list.",
    registerAssociation: "Register association",
    registering: "Registering…",
    associationSuccess:
      "File associations registered. If the context menu does not refresh immediately, reopen Explorer or restart once.",
    associationFailed: "Failed to register file associations",
    done: "Done",
  },
  export: {
    html: "Export HTML",
    htmlHint: "Self-contained · images/math/diagrams included",
    docx: "Export DOCX",
    docxHint: "Word document",
    docxRequiresPandoc: "Requires pandoc",
    pdf: "Export PDF",
    pdfHint: "Edge headless · WYSIWYG",
    pdfNoEdge: "Edge not detected (you can choose it when exporting)",
    print: "Print / System PDF",
    printHint: "Browser print dialog",
    generatingPdf: "Generating PDF…",
    generatingDocx: "Generating DOCX…",
    exportedPdf: "PDF exported",
    exportedDocx: "DOCX exported",
    exportFailed: "Export failed",
    exportBusy: "Exporting…",
    exportShortcut: "Export",
    chooseEdgePath: "Choose msedge.exe path",
    edgeChrome: "Edge / Chrome",
    pdfFailed: "Failed to export PDF",
    docxFailed: "Failed to export DOCX",
    dialogHtml: "Export as HTML",
    dialogPdf: "Export as PDF",
    dialogDocx: "Export as DOCX",
    wordDocument: "Word document",
  },
  editor: {
    edit: "Edit",
    preview: "Preview",
    save: "Save",
    saveAs: "Save As",
    unsaved: "Unsaved",
    saved: "Saved",
    saveFailed: "Failed to save",
    unsavedTitle: "Unsaved changes",
    unsavedMessage:
      "The current file has unsaved changes. Choose whether to save, discard, or cancel before continuing.",
    saveAndContinue: "Save and continue",
    discardAndContinue: "Discard and continue",
    cancel: "Cancel",
    externalChangedTitle: "File changed externally",
    externalChangedMessage:
      "The current file changed on disk. Reload the disk version or keep your unsaved edits.",
    reloadFromDisk: "Reload",
    keepEditing: "Keep editing",
    previewBeforeExport: "Switch to preview mode before exporting or printing",
    newFile: "New Markdown file",
    created: "Created",
    createFailed: "Failed to create",
  },
  toc: {
    title: "Outline",
    empty: "(No headings)",
    expandAll: "Expand All",
    collapseAll: "Collapse All",
  },
  tabs: {
    close: "Close",
    closeOthers: "Close Others",
    closeAll: "Close All",
    refresh: "Reload",
    copyPath: "Copy Path",
    copiedPath: "Path copied",
    copyFailed: "Copy failed",
  },
  pdfStyle: {
    title: "PDF Export Style",
    cat: {
      doc: "Document",
      dev: "Developer",
      creative: "Creative",
    },
    template: "Template",
    applyTemplate: "Apply template",
    custom: "Custom",
    bodyFont: "Body font",
    headingFont: "Heading font",
    codeFont: "Code font",
    fontSize: "Font size",
    lineHeight: "Line height",
    density: "Spacing density",
    compact: "Compact",
    standard: "Standard",
    loose: "Loose",
    textColor: "Text color",
    headingColor: "Heading color",
    linkColor: "Link color",
    codeBg: "Code background",
    bgColor: "Page background",
    pageMargin: "Page margin",
    pageSize: "Page size",
    a4: "A4",
    letter: "Letter",
    portrait: "Portrait",
    landscape: "Landscape",
    reset: "Reset",
    previewNote:
      "The font, color and template settings above are applied when exporting PDF or printing.",
    preview: "Live preview",
    sampleText: "Type Markdown here as a preview sample (editable)",
    compare: "Light / Dark compare",
    light: "Light",
    dark: "Dark",
    tpl: {
      default: "Modern (default)",
      minimal: "Minimal",
      academic: "Academic",
      eyeCare: "Eye-care",
      businessBlue: "Business Blue",
      classicBrown: "Classic Brown",
      dark: "Dark",
      github: "GitHub",
      terminalGreen: "Terminal Green",
      dracula: "Dracula Purple",
      vsCodeBlue: "VS Code Blue",
      sakuraPink: "Sakura Pink",
      lavender: "Lavender",
      oceanCyan: "Ocean Cyan",
      sunsetOrange: "Sunset Orange",
      forestMoss: "Forest Moss",
      nord: "Nord",
      solarizedLight: "Solarized Light",
      solarizedDark: "Solarized Dark",
      gruvboxLight: "Gruvbox Light",
      gruvboxDark: "Gruvbox Dark",
      monokai: "Monokai",
      catppuccin: "Catppuccin",
      oneDark: "One Dark",
    },
  },
  errors: {
    readFailed: "Failed to read file",
    fileExists: "File already exists. Choose another name.",
  },
  shortcuts: {
    title: "Shortcuts",
    hint: "View and customize all shortcuts",
    view: "View shortcuts",
    categoryGlobal: "Global",
    categoryEditor: "Editor",
    categoryFind: "Find",
    recording: "Press a key combo…",
    conflict: 'Conflicts with "{name}"',
    invalid: "Ctrl modifier required",
    resetAll: "Reset all",
    reset: "Reset",
    done: "Done",
    toggleMode: "Toggle preview / edit mode",
    newFile: "New file",
    openFile: "Open file",
    searchPanel: "Full-text search",
    save: "Save",
    saveAs: "Save as",
    settings: "Reading settings",
    print: "Print",
    zoomIn: "Increase font size",
    zoomOut: "Decrease font size",
    zoomReset: "Reset font size",
    find: "Find",
    replace: "Replace",
    goToLine: "Go to line",
    bold: "Bold",
    italic: "Italic",
    underline: "Underline",
    highlight: "Highlight",
    inlineCode: "Inline code",
    close: "Close find / settings / dialogs",
    zoomWheel: "Scroll to zoom font",
    closeTab: "Close current tab",
    nextTab: "Next tab",
    prevTab: "Previous tab",
    nextTabRight: "Next tab (right)",
    prevTabLeft: "Previous tab (left)",
  },
} satisfies MessageSchema;

export default enUS;
